...
def main():
	...
	if mode == 'encrypt':
		message = '''" peaceful life is a must "'''
		pubKeyFilename = 'abc_pubkey.txt'
		...
		privKeyFilename = 'abc_privkey.txt'
...
def getBlocksFromText(message, blockSize=DEFAULT_BLOCK_SIZE):
	...
	#messageBytes = message.encode("ascii", "ignore") # convert the string to bytes
	messageBytes = map(ord, message)
	blockInts = []
	for blockStart in range(0, len(messageBytes), blockSize):
		...
		s = messageBytes[i] * (BYTE_SIZE ** (i % blockSize))
		#s.split()
